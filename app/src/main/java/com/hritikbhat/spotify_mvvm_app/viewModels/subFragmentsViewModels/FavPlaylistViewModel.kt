...
import com.hritikbhat.spotify_mvvm_app.utils.Classes.CustomPlaylistManager
...
class FavPlaylistViewModel() : ViewModel() {
    ...
    private var customPlaylistManager = CustomPlaylistManager(apiService)
    ...
    suspend fun getUserFavPlaylist(fQ: FavPlaylistQuery): OperationResult<favPlaylists>{
         return playlistFavouritesManager.getUserFavPlaylist(fQ)
    }
    ...
    suspend fun deleteSongFromPlaylist(fQ: AddSongPlaylistQuery): OperationResult<FavTransactionResp> {
        return customPlaylistSongManager.deleteSongFromPlaylist(fQ)
    }
